...
EXPERIMENT_NAME = "spectral_moisture_refine_smooth_blend_ensemble"
...
ENSEMBLE_WEIGHT_CANDIDATES = tuple(float(x) for x in np.linspace(0.0, 1.0, 51))
...
def apply_ensemble_postprocess(predictions: np.ndarray, group_keys: pd.Series, method_name: str) -> np.ndarray:
    if method_name in {"none", "cummin", "isotonic"}:
    ...
    raise ValueError(f"Unsupported ensemble postprocess: {method_name}")


def select_best_ensemble_postprocess(
    predictions: np.ndarray,
    y_true: np.ndarray,
    group_keys: pd.Series,
) -> tuple[str, np.ndarray, dict[str, float]]:
    method_names = ["none", "cummin", "isotonic"] + [f"smooth_{smooth_lambda:g}" for smooth_lambda in SMOOTH_LAMBDA_CANDIDATES]
    candidates = {}
    for method_name in method_names:
        candidates[method_name] = apply_ensemble_postprocess(predictions, group_keys, method_name)
    scores = {name: compute_rmse(y_true, pred) for name, pred in candidates.items()}
    best_method = min(scores, key=scores.get)
    return best_method, candidates[best_method], scores
...
# %%
def run_non_nested_pipeline(alpha: float) -> dict[str, object]:
    train_pred_raw = generate_base_oof_predictions(alpha)
    ...
    delta_train_df, tail_train_df = build_refine_training_frames(
        train_meta_df,
        raw_predictions=train_pred_raw,
    ...
    best_smooth_lambda, train_pred_smooth, smooth_scores = select_best_smooth_lambda(
        train_pred_raw,
        y_true=y_train_array,
        group_keys=train_meta_df["group_key"],
    )
    test_pred_smooth = apply_groupwise_monotone_smooth(
        test_pred_raw,
        test_meta_df["group_key"],
        smooth_lambda=best_smooth_lambda,
    )

    blend_feature_train = build_blend_feature_frame(
        train_meta_df,
        raw_pred=train_pred_raw,
        backward_pred=train_pred_backward,
        smooth_pred=train_pred_smooth,
    )
    blend_feature_test = build_blend_feature_frame(
        test_meta_df,
        raw_pred=test_pred_raw,
        backward_pred=test_pred_backward,
        smooth_pred=test_pred_smooth,
    )
    blend_feature_cols = blend_feature_train.columns.tolist()
    ideal_blend_weight = compute_ideal_blend_weight(
        y_train_array,
        backward_pred=train_pred_backward,
        smooth_pred=train_pred_smooth,
    )
    ...
    blend_model = build_blend_model()
    blend_model.fit(blend_feature_train[blend_feature_cols], ideal_blend_weight)
    train_weight_pred = np.clip(blend_model.predict(blend_feature_train[blend_feature_cols]), 0.0, 1.0)
    test_weight_pred = np.clip(blend_model.predict(blend_feature_test[blend_feature_cols]), 0.0, 1.0)

    train_pred_blend_raw = train_weight_pred * train_pred_backward + (1.0 - train_weight_pred) * train_pred_smooth
    test_pred_blend_raw = test_weight_pred * test_pred_backward + (1.0 - test_weight_pred) * test_pred_smooth
    ...
    return {
        ...
        "train_pred_backward": train_pred_backward,
        "train_pred_smooth": train_pred_smooth,
        "train_weight_pred": train_weight_pred,
        "train_pred_blend_raw": train_pred_blend_raw,
        "train_pred_final": train_pred_final,
        "test_pred_raw": test_pred_raw,
        "test_pred_backward": test_pred_backward,
        "test_pred_smooth": test_pred_smooth,
        "test_weight_pred": test_weight_pred,
        "test_pred_blend_raw": test_pred_blend_raw,
        "test_pred_final": test_pred_final,
        "best_smooth_lambda": best_smooth_lambda,
        "smooth_scores": smooth_scores,
...
def run_nested_pipeline(alpha: float) -> dict[str, object]:
    ...
    for fold_no, (fit_idx, valid_idx) in enumerate(cv_splits, start=1):
        fold_result = run_nested_outer_fold(fold_no, fit_idx, valid_idx, alpha)
        ...
        print(
            f"Nested fold {fold_no}: smooth_lambda={float(fold_result['smooth_lambda']):g}, "
    ...
    return {
        ...
        "test_pred_final": test_pred_final,
        "smooth_scores": smooth_scores,
        "smooth_lambda_choices": smooth_lambda_choices,
        "final_method": final_method,
        "final_scores": final_scores,
...
def search_best_ensemble(
    train_pred_non_nested: np.ndarray,
    train_pred_nested: np.ndarray,
    y_true: np.ndarray,
    group_keys: pd.Series,
    ...
    for weight_non_nested in ENSEMBLE_WEIGHT_CANDIDATES:
        blended_raw = (
            weight_non_nested * np.asarray(train_pred_non_nested, dtype=float).reshape(-1)
            + (1.0 - weight_non_nested) * np.asarray(train_pred_nested, dtype=float).reshape(-1)
        )
        best_method, blended_final, score_by_method = select_best_ensemble_postprocess(
            blended_raw,
            y_true=y_true,
            group_keys=group_keys,
        )
        best_rmse = float(score_by_method[best_method])
        search_rows.append(
            {
                "weight_non_nested": float(weight_non_nested),
                "weight_nested": float(1.0 - weight_non_nested),
                "best_method": best_method,
                "best_rmse": best_rmse,
            }
        )
        if best_result is None or best_rmse < float(best_result["best_rmse"]):
            best_result = {
                "weight_non_nested": float(weight_non_nested),
                "weight_nested": float(1.0 - weight_non_nested),
                "best_method": best_method,
                "best_rmse": best_rmse,
                "train_pred_blended_raw": blended_raw,
                "train_pred_final": blended_final,
                "score_by_method": score_by_method,
            }

    if best_result is None:
        raise RuntimeError("ensemble search failed")

    search_df = pd.DataFrame(search_rows).sort_values("best_rmse").reset_index(drop=True)
...
mlflow.set_tracking_uri(f"file:{MLRUNS_DIR}")
mlflow.set_experiment(EXPERIMENT_NAME)
with mlflow.start_run(run_name=f"refine_smooth_blend_ensemble_{DATE}") as run:
    study = optuna.create_study(direction="minimize")
    study.optimize(objective, n_trials=N_TRIALS, show_progress_bar=True)
    best_alpha = float(study.best_params["alpha"])
    base_cv_rmse = float(study.best_value)
    ...
    non_nested_result = run_non_nested_pipeline(best_alpha)
    nested_result = run_nested_pipeline(best_alpha)
    fold_summary_df = pd.DataFrame(nested_result["fold_summary_df"]).copy()
    print(fold_summary_df)
    print(
        "Nested smooth lambda choices by fold:",
        summarize_choice_counts(list(nested_result["smooth_lambda_choices"])),
    )

    ensemble_best, ensemble_search_df = search_best_ensemble(
        train_pred_non_nested=np.asarray(non_nested_result["train_pred_final"], dtype=float),
        train_pred_nested=np.asarray(nested_result["train_pred_final"], dtype=float),
        y_true=y_train_array,
        group_keys=train_meta_df["group_key"],
    )
    print(ensemble_search_df.head(10))

    ensemble_weight_non_nested = float(ensemble_best["weight_non_nested"])
    ensemble_weight_nested = float(ensemble_best["weight_nested"])
    ensemble_method = str(ensemble_best["best_method"])
    ensemble_score_by_method = dict(ensemble_best["score_by_method"])
    train_pred_blend_raw = np.asarray(ensemble_best["train_pred_blended_raw"], dtype=float)
    train_pred_final = np.asarray(ensemble_best["train_pred_final"], dtype=float)
    test_pred_blend_raw = (
        ensemble_weight_non_nested * np.asarray(non_nested_result["test_pred_final"], dtype=float)
        + ensemble_weight_nested * np.asarray(nested_result["test_pred_final"], dtype=float)
    )
    test_pred_final = apply_ensemble_postprocess(
        test_pred_blend_raw,
        group_keys=test_meta_df["group_key"],
        method_name=ensemble_method,
    )

    train_pred_raw = np.asarray(nested_result["train_pred_raw"], dtype=float)
    test_pred_raw = np.asarray(nested_result["test_pred_raw"], dtype=float)

    train_non_nested_rmse = compute_rmse(y_train_array, np.asarray(non_nested_result["train_pred_final"], dtype=float))
    train_nested_rmse = compute_rmse(y_train_array, np.asarray(nested_result["train_pred_final"], dtype=float))
    train_ensemble_blend_rmse = compute_rmse(y_train_array, train_pred_blend_raw)
    train_final_rmse = compute_rmse(y_train_array, train_pred_final)
    print(
        f"Train RMSE non_nested={train_non_nested_rmse:.4f}, nested={train_nested_rmse:.4f}, "
        f"ensemble_raw={train_ensemble_blend_rmse:.4f}, ensemble_final={train_final_rmse:.4f}"
    )
    print("Ensemble postprocess RMSE:", {k: round(v, 4) for k, v in ensemble_score_by_method.items()})
    print(
        f"Best ensemble weight: non_nested={ensemble_weight_non_nested:.2f}, "
        f"nested={ensemble_weight_nested:.2f}, method={ensemble_method}"
    )

    train_non_nested_violations = count_monotonic_violations(
        np.asarray(non_nested_result["train_pred_final"], dtype=float),
        train_meta_df["group_key"],
    )
    train_nested_violations = count_monotonic_violations(
        np.asarray(nested_result["train_pred_final"], dtype=float),
        train_meta_df["group_key"],
    )
    train_ensemble_raw_violations = count_monotonic_violations(train_pred_blend_raw, train_meta_df["group_key"])
    train_final_violations = count_monotonic_violations(train_pred_final, train_meta_df["group_key"])
    test_non_nested_violations = count_monotonic_violations(
        np.asarray(non_nested_result["test_pred_final"], dtype=float),
        test_meta_df["group_key"],
    )
    test_nested_violations = count_monotonic_violations(
        np.asarray(nested_result["test_pred_final"], dtype=float),
        test_meta_df["group_key"],
    )
    test_ensemble_raw_violations = count_monotonic_violations(test_pred_blend_raw, test_meta_df["group_key"])
    test_final_violations = count_monotonic_violations(test_pred_final, test_meta_df["group_key"])
    print(
        f"Violations train non_nested={train_non_nested_violations}, train nested={train_nested_violations}, "
        f"train ensemble_raw={train_ensemble_raw_violations}, train final={train_final_violations}"
    )
    print(
        f"Violations test non_nested={test_non_nested_violations}, test nested={test_nested_violations}, "
        f"test ensemble_raw={test_ensemble_raw_violations}, test final={test_final_violations}"
    )

    submit_df = build_submit_df(test_ids, test_pred_final)
    rmse_tag = int(round(train_final_rmse * 10000))
    submit_path = os.path.join(SUBMIT_DIR, f"submit_csv_09_refine_smooth_blend_ensemble_{DATE}_{rmse_tag:04d}.csv")
    submit_df.to_csv(submit_path, index=False, header=False, encoding="utf-8-sig")
    print(f"saved: {submit_path}")
    ...
    detail_df = pd.DataFrame(
        {
            ...
            "sample_order_in_species": test_meta_df["sample_order_in_species"],
            "raw_prediction": test_pred_raw,
            "non_nested_final_prediction": np.asarray(non_nested_result["test_pred_final"], dtype=float),
            "nested_final_prediction": np.asarray(nested_result["test_pred_final"], dtype=float),
            "ensemble_raw_prediction": test_pred_blend_raw,
            "ensemble_final_prediction": test_pred_final,
            "ensemble_weight_non_nested": ensemble_weight_non_nested,
            "ensemble_weight_nested": ensemble_weight_nested,
            "ensemble_method": ensemble_method,
            "nested_smooth_lambda_choices": summarize_choice_counts(list(nested_result["smooth_lambda_choices"])),
        }
    )
    detail_path = os.path.join(SUBMIT_DIR, f"test_prediction_detail_09_refine_smooth_blend_ensemble_{DATE}.csv")
    detail_df.to_csv(detail_path, index=False, encoding="utf-8-sig")
    fold_summary_path = os.path.join(SUBMIT_DIR, f"fold_summary_09_refine_smooth_blend_ensemble_{DATE}.csv")
    fold_summary_df.to_csv(fold_summary_path, index=False, encoding="utf-8-sig")
    ensemble_search_path = os.path.join(SUBMIT_DIR, f"ensemble_search_09_refine_smooth_blend_ensemble_{DATE}.csv")
    ensemble_search_df.to_csv(ensemble_search_path, index=False, encoding="utf-8-sig")
    ...
    mlflow.log_params(
        {
            ...
            "prediction_lower_bound": PREDICTION_LOWER_BOUND,
            "refine_target_method": REFINE_TARGET_METHOD,
            "non_nested_final_method": str(non_nested_result["final_method"]),
            "nested_final_method": str(nested_result["final_method"]),
            "ensemble_method": ensemble_method,
            "ensemble_weight_non_nested": ensemble_weight_non_nested,
            "ensemble_weight_nested": ensemble_weight_nested,
            "non_nested_blend_feature_count": int(non_nested_result["blend_feature_count"]),
            "nested_blend_feature_count": int(nested_result["blend_feature_count"]),
            "nested_smooth_lambda_choices": summarize_choice_counts(list(nested_result["smooth_lambda_choices"])),
        }
    )
    mlflow.log_metric("base_cv_rmse", base_cv_rmse)
    mlflow.log_metric("train_non_nested_rmse", train_non_nested_rmse)
    mlflow.log_metric("train_nested_rmse", train_nested_rmse)
    mlflow.log_metric("train_ensemble_raw_rmse", train_ensemble_blend_rmse)
    mlflow.log_metric("train_final_rmse", train_final_rmse)
    mlflow.log_metric("train_non_nested_violations", train_non_nested_violations)
    mlflow.log_metric("train_nested_violations", train_nested_violations)
    mlflow.log_metric("train_ensemble_raw_violations", train_ensemble_raw_violations)
    mlflow.log_metric("train_final_violations", train_final_violations)
    mlflow.log_metric("test_non_nested_violations", test_non_nested_violations)
    mlflow.log_metric("test_nested_violations", test_nested_violations)
    mlflow.log_metric("test_ensemble_raw_violations", test_ensemble_raw_violations)
    mlflow.log_metric("test_final_violations", test_final_violations)
    for _, row in fold_summary_df.iterrows():
        mlflow.log_metric(f"fold_{int(row['fold'])}_blend_raw_valid_rmse", float(row["blend_raw_valid_rmse"]))
    for method_name, score in dict(non_nested_result["smooth_scores"]).items():
        mlflow.log_metric(f"non_nested_smooth_rmse_{method_name}", float(score))
    for method_name, score in dict(nested_result["smooth_scores"]).items():
        mlflow.log_metric(f"nested_smooth_rmse_{method_name}", float(score))
    for method_name, score in ensemble_score_by_method.items():
        mlflow.log_metric(f"ensemble_rmse_{method_name}", float(score))
    mlflow.log_artifact(submit_path)
    mlflow.log_artifact(detail_path)
    mlflow.log_artifact(fold_summary_path)
    mlflow.log_artifact(ensemble_search_path)
    mlflow.log_artifact(__file__)
    print(f"mlflow run_id: {run.info.run_id}")
...
# %%
plot_raw_vs_final(train_meta_df, train_pred_raw, train_pred_final, "Train", ensemble_method)
plot_raw_vs_final(test_meta_df, test_pred_raw, test_pred_final, "Test", ensemble_method)
plot_prediction_trend_by_species(
    train_meta_df,
    train_pred_final,
    f"Train: ensemble predicted moisture trend ({ensemble_method})",
)
plot_prediction_trend_by_species(
    test_meta_df,
    test_pred_final,
    f"Test: ensemble predicted moisture trend ({ensemble_method})",
)
