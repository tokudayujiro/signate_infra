...
# 08 の最終予測の近傍だけを小さく混ぜる sparse ensemble を作る
...
EXPERIMENT_NAME = "spectral_moisture_08_nearby_ensemble"
...
NEARBY_VARIANT_SMOOTH_LAMBDAS = (0.1, 0.3, 1.0, 3.0, 10.0, 30.0)
NEARBY_BLEND_ALPHA_CANDIDATES = (0.05, 0.1, 0.15, 0.2, 0.25, 0.3)
NEARBY_POSTPROCESS_CANDIDATES = ("none", "cummin", "isotonic", "smooth_0.3", "smooth_1", "smooth_3")
NEARBY_ENSEMBLE_MAX_STEPS = 3
NEARBY_BASE_DRIFT_PENALTY = 0.12
NEARBY_MIN_IMPROVEMENT = 1e-6
...
def apply_nearby_postprocess(predictions: np.ndarray, group_keys: pd.Series, method_name: str) -> np.ndarray:
    """近傍 ensemble 用の後処理を適用する。"""
...
def build_nearby_variant_pool(
    train_pred_final: np.ndarray,
    test_pred_final: np.ndarray,
    train_pred_backward: np.ndarray,
    test_pred_backward: np.ndarray,
    train_pred_smooth: np.ndarray,
    test_pred_smooth: np.ndarray,
    train_pred_blend_raw: np.ndarray,
    test_pred_blend_raw: np.ndarray,
) -> dict[str, dict[str, np.ndarray]]:
    """08 の最終予測の近くにある候補系列をまとめる。"""
    variant_pool: dict[str, dict[str, np.ndarray]] = {
        "stage0_final": {
            "train": np.asarray(train_pred_final, dtype=float).reshape(-1),
            "test": np.asarray(test_pred_final, dtype=float).reshape(-1),
        },
        "backward_isotonic": {
            "train": apply_groupwise_isotonic(train_pred_backward, train_meta_df["group_key"]),
            "test": apply_groupwise_isotonic(test_pred_backward, test_meta_df["group_key"]),
        },
        "smooth_selected": {
            "train": np.asarray(train_pred_smooth, dtype=float).reshape(-1),
            "test": np.asarray(test_pred_smooth, dtype=float).reshape(-1),
        },
        "blend_cummin": {
            "train": apply_groupwise_cummin(train_pred_blend_raw, train_meta_df["group_key"]),
            "test": apply_groupwise_cummin(test_pred_blend_raw, test_meta_df["group_key"]),
        },
        "blend_isotonic": {
            "train": apply_groupwise_isotonic(train_pred_blend_raw, train_meta_df["group_key"]),
            "test": apply_groupwise_isotonic(test_pred_blend_raw, test_meta_df["group_key"]),
        },
    }

    for smooth_lambda in NEARBY_VARIANT_SMOOTH_LAMBDAS:
        label = f"blend_smooth_{smooth_lambda:g}"
        variant_pool[label] = {
            "train": apply_groupwise_monotone_smooth(train_pred_blend_raw, train_meta_df["group_key"], smooth_lambda),
            "test": apply_groupwise_monotone_smooth(test_pred_blend_raw, test_meta_df["group_key"], smooth_lambda),
        }
    return variant_pool


def compute_nearby_objective(
    y_true: np.ndarray,
    stage0_pred: np.ndarray,
    candidate_pred: np.ndarray,
) -> tuple[float, float, float]:
    """RMSE と 08 からの乖離を合わせた評価値を返す。"""
    rmse = compute_rmse(y_true, candidate_pred)
    drift = compute_rmse(stage0_pred, candidate_pred)
    objective = float(rmse + NEARBY_BASE_DRIFT_PENALTY * drift)
    return objective, rmse, drift


def search_nearby_ensemble(
    y_true: np.ndarray,
    stage0_train_pred: np.ndarray,
    stage0_test_pred: np.ndarray,
    variant_pool: dict[str, dict[str, np.ndarray]],
) -> tuple[dict[str, object], pd.DataFrame]:
    """08 の最終予測の近傍で greedy に ensemble を探索する。"""
    current_train = np.asarray(stage0_train_pred, dtype=float).reshape(-1).copy()
    current_test = np.asarray(stage0_test_pred, dtype=float).reshape(-1).copy()
    history_rows: list[dict[str, object]] = []

    current_objective, current_rmse, current_drift = compute_nearby_objective(y_true, stage0_train_pred, current_train)
    history_rows.append(
        {
            "step": 0,
            "variant": "stage0_final",
            "alpha": 0.0,
            "postprocess": "none",
            "objective": current_objective,
            "train_rmse": current_rmse,
            "drift_from_stage0": current_drift,
            "train_violations": int(count_monotonic_violations(current_train, train_meta_df["group_key"])),
            "test_violations": int(count_monotonic_violations(current_test, test_meta_df["group_key"])),
        }
    )

    for step_no in range(1, NEARBY_ENSEMBLE_MAX_STEPS + 1):
        best_local: dict[str, object] | None = None

        for variant_name, payload in variant_pool.items():
            if variant_name == "stage0_final":
                continue
            variant_train = np.asarray(payload["train"], dtype=float).reshape(-1)
            variant_test = np.asarray(payload["test"], dtype=float).reshape(-1)

            for alpha in NEARBY_BLEND_ALPHA_CANDIDATES:
                blended_train_raw = (1.0 - alpha) * current_train + alpha * variant_train
                blended_test_raw = (1.0 - alpha) * current_test + alpha * variant_test

                for method_name in NEARBY_POSTPROCESS_CANDIDATES:
                    final_train = apply_nearby_postprocess(blended_train_raw, train_meta_df["group_key"], method_name)
                    final_test = apply_nearby_postprocess(blended_test_raw, test_meta_df["group_key"], method_name)
                    objective, train_rmse, drift = compute_nearby_objective(y_true, stage0_train_pred, final_train)
                    candidate = {
                        "step": step_no,
                    ...
                    if best_local is None or float(candidate["objective"]) < float(best_local["objective"]):
                        best_local = candidate

        if best_local is None:
            break
        if float(best_local["objective"]) >= current_objective - NEARBY_MIN_IMPROVEMENT:
            break

        current_train = np.asarray(best_local["train_pred"], dtype=float).copy()
        current_test = np.asarray(best_local["test_pred"], dtype=float).copy()
        current_objective = float(best_local["objective"])
        current_rmse = float(best_local["train_rmse"])
        current_drift = float(best_local["drift_from_stage0"])
        history_rows.append(
            {
                "step": int(best_local["step"]),
                "variant": str(best_local["variant"]),
                "alpha": float(best_local["alpha"]),
                "postprocess": str(best_local["postprocess"]),
                "objective": current_objective,
                "train_rmse": current_rmse,
                "drift_from_stage0": current_drift,
                "train_violations": int(best_local["train_violations"]),
                "test_violations": int(best_local["test_violations"]),
            }
        )

    result = {
        "train_pred": current_train,
        "test_pred": current_test,
        "objective": current_objective,
        "train_rmse": current_rmse,
        "drift_from_stage0": current_drift,
...
def plot_nearby_history(history_df: pd.DataFrame) -> None:
    """近傍 ensemble の各ステップで指標がどう変わったかを描画する。"""
    if history_df.empty:
        return

    plt.figure(figsize=(10, 5))
    sns.lineplot(data=history_df, x="step", y="train_rmse", marker="o", label="train_rmse")
    sns.lineplot(data=history_df, x="step", y="objective", marker="o", label="objective")
    plt.xticks(history_df["step"])
    plt.xlabel("step")
    plt.ylabel("score")
    plt.title("Nearby ensemble search history")
    plt.tight_layout()
    plt.show()
...
with mlflow.start_run(run_name=f"nearby_ensemble_{DATE}") as run:
    ...
    nearby_variant_pool = build_nearby_variant_pool(
    ...
    nearby_result, nearby_history_df = search_nearby_ensemble(
        y_true=y_train_array,
        stage0_train_pred=train_pred_final,
        stage0_test_pred=test_pred_final,
        variant_pool=nearby_variant_pool,
    )
    selected_train_pred = np.asarray(nearby_result["train_pred"], dtype=float)
    selected_test_pred = np.asarray(nearby_result["test_pred"], dtype=float)
    selected_objective = float(nearby_result["objective"])
    selected_drift = float(nearby_result["drift_from_stage0"])
    ...
    print(
        ...
        f"stage0_final={train_final_rmse:.4f}, selected={selected_train_rmse:.4f}"
    )
    print("Stage0 final postprocess RMSE:", {k: round(v, 4) for k, v in final_scores.items()})
    print(nearby_history_df)
    print(
        f"Nearby objective={selected_objective:.4f}, drift_from_stage0={selected_drift:.4f}"
    )
    ...
    print(
        ...
        f"stage0_final={train_final_violations}, selected={selected_train_violations}, "
        f"test raw={test_raw_violations}, test blend_raw={test_blend_raw_violations}, "
        f"stage0_final={test_final_violations}, selected={selected_test_violations}"
    )

    submit_df = build_submit_df(test_ids, selected_test_pred)
    rmse_tag = int(round(selected_train_rmse * 10000))
    submit_path = os.path.join(SUBMIT_DIR, f"submit_csv_12_08_nearby_ensemble_{DATE}_{rmse_tag:04d}.csv")
    ...
    detail_df = pd.DataFrame(
        {
            ...
            "nearby_objective": selected_objective,
            "nearby_drift_from_stage0": selected_drift,
            "smooth_lambda_choices": summarize_choice_counts(smooth_lambda_choices),
        }
    )
    for variant_name, payload in nearby_variant_pool.items():
        detail_df[f"{variant_name}_prediction"] = np.asarray(payload["test"], dtype=float)
    detail_path = os.path.join(SUBMIT_DIR, f"test_prediction_detail_12_08_nearby_ensemble_{DATE}.csv")
    detail_df.to_csv(detail_path, index=False, encoding="utf-8-sig")
    fold_summary_path = os.path.join(SUBMIT_DIR, f"fold_summary_12_08_nearby_ensemble_{DATE}.csv")
    fold_summary_df.to_csv(fold_summary_path, index=False, encoding="utf-8-sig")
    nearby_history_path = os.path.join(SUBMIT_DIR, f"nearby_history_12_08_nearby_ensemble_{DATE}.csv")
    nearby_history_df.to_csv(nearby_history_path, index=False, encoding="utf-8-sig")
    ...
    mlflow.log_params(
        {
            ...
            "smooth_lambda_choices": summarize_choice_counts(smooth_lambda_choices),
            "nearby_max_steps": NEARBY_ENSEMBLE_MAX_STEPS,
            "nearby_drift_penalty": NEARBY_BASE_DRIFT_PENALTY,
        }
    )
    ...
    mlflow.log_metric("train_selected_objective", selected_objective)
    mlflow.log_metric("train_selected_drift_from_stage0", selected_drift)
    ...
    for _, row in nearby_history_df.iterrows():
        mlflow.log_metric(f"nearby_step_{int(row['step'])}_train_rmse", float(row["train_rmse"]))
    mlflow.log_artifact(submit_path)
    mlflow.log_artifact(detail_path)
    mlflow.log_artifact(fold_summary_path)
    mlflow.log_artifact(nearby_history_path)
...
# %%
plot_raw_vs_final(train_meta_df, train_pred_final, selected_train_pred, "Train", "nearby_ensemble")
plot_raw_vs_final(test_meta_df, test_pred_final, selected_test_pred, "Test", "nearby_ensemble")
final_method = "nearby_ensemble"
...
plot_nearby_history(nearby_history_df)
