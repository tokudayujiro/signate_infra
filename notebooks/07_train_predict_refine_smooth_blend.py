...
EXPERIMENT_NAME = "spectral_moisture_refine_smooth_blend"
...
with mlflow.start_run(run_name=f"refine_smooth_blend_{DATE}") as run:
    ...
    print("Smooth RMSE by lambda:", {k: round(v, 4) for k, v in smooth_scores.items()})

    blend_feature_train = build_blend_feature_frame(
        train_meta_df,
        raw_pred=base_oof_pred,
    ...
    ideal_blend_weight = compute_ideal_blend_weight(y_train_array, train_pred_backward, train_pred_smooth)
    ...
    print(
        f"Violations train raw={train_raw_violations}, train final={train_final_violations}, "
        f"test raw={test_raw_violations}, test final={test_final_violations}"
    )

    submit_df = build_submit_df(test_ids, test_pred_final)
    rmse_tag = int(round(train_final_rmse * 10000))
    submit_path = os.path.join(SUBMIT_DIR, f"submit_csv_07_refine_smooth_blend_{DATE}_{rmse_tag:04d}.csv")
    ...
    detail_path = os.path.join(SUBMIT_DIR, f"test_prediction_detail_07_refine_smooth_blend_{DATE}.csv")
...
# %%
plot_raw_vs_final(train_meta_df, base_oof_pred, train_pred_final, "Train", final_method)
