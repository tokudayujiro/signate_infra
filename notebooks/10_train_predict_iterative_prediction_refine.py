...
# backward refine と monotone smooth の最終予測をさらに反復補正する
...
EXPERIMENT_NAME = "spectral_moisture_iterative_prediction_refine"
...
ITERATIVE_REFINE_STEPS = 3
ITERATIVE_UPDATE_RATE = 0.8
ITERATIVE_LEARNING_RATE = 0.05
ITERATIVE_MAX_DEPTH = 3
ITERATIVE_MAX_ITER = 300
ITERATIVE_MIN_SAMPLES_LEAF = 20
ITERATIVE_L2_REGULARIZATION = 0.1
...
def apply_iterative_postprocess(predictions: np.ndarray, group_keys: pd.Series, method_name: str) -> np.ndarray:
    """反復補正ステージ用の後処理を適用する。"""
    ...
    return apply_final_postprocess(predictions, group_keys, method_name)


def select_best_iterative_postprocess(
    predictions: np.ndarray,
    y_true: np.ndarray,
    group_keys: pd.Series,
) -> tuple[str, np.ndarray, dict[str, float]]:
    """反復補正ステージで使う後処理を RMSE で選ぶ。"""
    candidates: dict[str, np.ndarray] = {}
    method_names = ["none", "cummin", "isotonic"] + [f"smooth_{v:g}" for v in SMOOTH_LAMBDA_CANDIDATES]
    for method_name in method_names:
        candidates[method_name] = apply_iterative_postprocess(predictions, group_keys, method_name)
    scores = {name: compute_rmse(y_true, pred) for name, pred in candidates.items()}
    best_method = min(scores, key=scores.get)
    return best_method, candidates[best_method], scores


def build_iterative_model() -> HistGradientBoostingRegressor:
    """予測値から次段の補正量を学習するモデルを作る。"""
    return HistGradientBoostingRegressor(
        learning_rate=ITERATIVE_LEARNING_RATE,
        max_depth=ITERATIVE_MAX_DEPTH,
        max_iter=ITERATIVE_MAX_ITER,
        min_samples_leaf=ITERATIVE_MIN_SAMPLES_LEAF,
        l2_regularization=ITERATIVE_L2_REGULARIZATION,
        random_state=RANDOM_STATE,
    )


def build_iterative_feature_frame(
    meta_df: pd.DataFrame,
    ...
    blend_raw_pred: np.ndarray,
) -> pd.DataFrame:
    """現在の予測列と stage 0 の参照列から反復補正用特徴量を作る。"""
    current_pred = np.asarray(current_pred, dtype=float).reshape(-1)
    initial_pred = np.asarray(initial_pred, dtype=float).reshape(-1)
    raw_pred = np.asarray(raw_pred, dtype=float).reshape(-1)
    backward_pred = np.asarray(backward_pred, dtype=float).reshape(-1)
    smooth_pred = np.asarray(smooth_pred, dtype=float).reshape(-1)
    blend_raw_pred = np.asarray(blend_raw_pred, dtype=float).reshape(-1)

    feature_df = meta_df[["sample_order_in_species", "group_size", "remaining_steps", "order_ratio", "remaining_ratio"]].copy()
    feature_df["current_pred"] = current_pred
    feature_df["initial_pred"] = initial_pred
    feature_df["raw_pred"] = raw_pred
    feature_df["backward_pred"] = backward_pred
    feature_df["smooth_pred"] = smooth_pred
    feature_df["blend_raw_pred"] = blend_raw_pred
    feature_df["current_minus_initial"] = current_pred - initial_pred
    feature_df["current_minus_raw"] = current_pred - raw_pred
    feature_df["current_minus_backward"] = current_pred - backward_pred
    feature_df["current_minus_smooth"] = current_pred - smooth_pred
    feature_df["current_minus_blend_raw"] = current_pred - blend_raw_pred

    prev_current = np.zeros_like(current_pred)
    next_current = np.zeros_like(current_pred)
    next2_current = np.zeros_like(current_pred)
    current_delta_prev = np.zeros_like(current_pred)
    current_delta_next = np.zeros_like(current_pred)
    current_delta_next2 = np.zeros_like(current_pred)
    curvature = np.zeros_like(current_pred)
    future_min = np.zeros_like(current_pred)
    future_max = np.zeros_like(current_pred)
    future_mean = np.zeros_like(current_pred)
    future_std = np.zeros_like(current_pred)
    group_start = np.zeros_like(current_pred)
    group_end = np.zeros_like(current_pred)
    group_range = np.zeros_like(current_pred)

    for group_idx in iter_group_indices(meta_df["group_key"]):
        group_values = current_pred[group_idx]
        group_cache = build_group_feature_cache(group_values)
        group_len = len(group_idx)

        prev_values = group_values.copy()
        prev_values[1:] = group_values[:-1]

        next_values = group_values.copy()
        next_values[:-1] = group_values[1:]

        next2_values = group_values.copy()
        if group_len > 2:
            next2_values[:-2] = group_values[2:]

        prev_current[group_idx] = prev_values
        next_current[group_idx] = next_values
        next2_current[group_idx] = next2_values
        current_delta_prev[group_idx] = group_values - prev_values
        current_delta_next[group_idx] = group_values - next_values
        current_delta_next2[group_idx] = group_values - next2_values
        curvature[group_idx] = prev_values - 2.0 * group_values + next_values
        future_min[group_idx] = np.asarray(group_cache["suffix_min"], dtype=float)
        future_max[group_idx] = np.asarray(group_cache["suffix_max"], dtype=float)
        future_mean[group_idx] = np.asarray(group_cache["suffix_mean"], dtype=float)
        future_std[group_idx] = np.asarray(group_cache["suffix_std"], dtype=float)
        group_start[group_idx] = float(group_cache["group_raw_start"])
        group_end[group_idx] = float(group_cache["group_raw_end"])
        group_range[group_idx] = float(group_cache["group_raw_range"])

    feature_df["prev_current"] = prev_current
    feature_df["next_current"] = next_current
    feature_df["next2_current"] = next2_current
    feature_df["current_delta_prev"] = current_delta_prev
    feature_df["current_delta_next"] = current_delta_next
    feature_df["current_delta_next2"] = current_delta_next2
    feature_df["current_curvature"] = curvature
    feature_df["future_current_min"] = future_min
    feature_df["future_current_max"] = future_max
    feature_df["future_current_mean"] = future_mean
    feature_df["future_current_std"] = future_std
    feature_df["group_current_start"] = group_start
    feature_df["group_current_end"] = group_end
    feature_df["group_current_range"] = group_range
    return feature_df


def run_iterative_refine_step(
    iteration_no: int,
    current_train_pred: np.ndarray,
    current_test_pred: np.ndarray,
    train_reference: dict[str, np.ndarray],
    test_reference: dict[str, np.ndarray],
) -> dict[str, object]:
    """現在の予測を入力にして、次段の補正予測を cross-fit で作る。"""
    train_feature_df = build_iterative_feature_frame(
        train_meta_df,
        current_pred=current_train_pred,
        initial_pred=train_reference["initial_pred"],
        raw_pred=train_reference["raw_pred"],
        backward_pred=train_reference["backward_pred"],
        smooth_pred=train_reference["smooth_pred"],
        blend_raw_pred=train_reference["blend_raw_pred"],
    )
    test_feature_df = build_iterative_feature_frame(
        test_meta_df,
        current_pred=current_test_pred,
        initial_pred=test_reference["initial_pred"],
        raw_pred=test_reference["raw_pred"],
        backward_pred=test_reference["backward_pred"],
        smooth_pred=test_reference["smooth_pred"],
        blend_raw_pred=test_reference["blend_raw_pred"],
    )
    feature_cols = train_feature_df.columns.tolist()
    residual_target = y_train_array - np.asarray(current_train_pred, dtype=float).reshape(-1)

    train_update_oof = np.zeros(len(train_feature_df), dtype=float)
    for fit_idx, valid_idx in cv_splits:
        stage_model = build_iterative_model()
        stage_model.fit(train_feature_df.iloc[fit_idx][feature_cols], residual_target[fit_idx])
        train_update_oof[valid_idx] = stage_model.predict(train_feature_df.iloc[valid_idx][feature_cols])

    full_stage_model = build_iterative_model()
    full_stage_model.fit(train_feature_df[feature_cols], residual_target)
    test_update = full_stage_model.predict(test_feature_df[feature_cols])

    next_train_raw = np.asarray(current_train_pred, dtype=float).reshape(-1) + ITERATIVE_UPDATE_RATE * train_update_oof
    next_test_raw = np.asarray(current_test_pred, dtype=float).reshape(-1) + ITERATIVE_UPDATE_RATE * test_update
    next_train_raw = np.clip(next_train_raw, PREDICTION_LOWER_BOUND, None)
    next_test_raw = np.clip(next_test_raw, PREDICTION_LOWER_BOUND, None)

    stage_method, next_train_final, stage_scores = select_best_iterative_postprocess(
        next_train_raw,
        y_true=y_train_array,
        group_keys=train_meta_df["group_key"],
    )
    next_test_final = apply_iterative_postprocess(
        next_test_raw,
        group_keys=test_meta_df["group_key"],
        method_name=stage_method,
    )

    return {
        "iteration": iteration_no,
        "feature_count": len(feature_cols),
        "update_std": float(np.std(train_update_oof)),
        "train_raw": next_train_raw,
        "test_raw": next_test_raw,
        "train_final": next_train_final,
        "test_final": next_test_final,
        "method": stage_method,
        "scores": stage_scores,
        "train_raw_rmse": compute_rmse(y_train_array, next_train_raw),
        "train_final_rmse": compute_rmse(y_train_array, next_train_final),
        "train_raw_violations": count_monotonic_violations(next_train_raw, train_meta_df["group_key"]),
        "train_final_violations": count_monotonic_violations(next_train_final, train_meta_df["group_key"]),
        "test_raw_violations": count_monotonic_violations(next_test_raw, test_meta_df["group_key"]),
        "test_final_violations": count_monotonic_violations(next_test_final, test_meta_df["group_key"]),
    }
...
def plot_iteration_rmse(iteration_summary_df: pd.DataFrame) -> None:
    """反復回数ごとの RMSE 推移を可視化する。"""
    if iteration_summary_df.empty:
        return

    plot_df = iteration_summary_df.copy()
    plt.figure(figsize=(10, 5))
    sns.lineplot(data=plot_df, x="stage_order", y="train_final_rmse", marker="o", label="train_final_rmse")
    sns.lineplot(data=plot_df, x="stage_order", y="train_raw_rmse", marker="o", label="train_raw_rmse")
    plt.xticks(plot_df["stage_order"], plot_df["stage_label"], rotation=20, ha="right")
    plt.xlabel("stage")
    plt.ylabel("RMSE")
    plt.title("Iterative refine RMSE by stage")
    plt.tight_layout()
    plt.show()
...
with mlflow.start_run(run_name=f"iterative_prediction_refine_{DATE}") as run:
    ...
    stage0_raw_rmse = compute_rmse(y_train_array, train_pred_blend_raw)
    stage0_final_rmse = compute_rmse(y_train_array, train_pred_final)
    stage0_raw_violations = count_monotonic_violations(train_pred_blend_raw, train_meta_df["group_key"])
    stage0_final_violations = count_monotonic_violations(train_pred_final, train_meta_df["group_key"])
    stage0_test_raw_violations = count_monotonic_violations(test_pred_blend_raw, test_meta_df["group_key"])
    stage0_test_final_violations = count_monotonic_violations(test_pred_final, test_meta_df["group_key"])

    train_reference = {
        "initial_pred": train_pred_final.copy(),
        "raw_pred": train_pred_raw.copy(),
        "backward_pred": train_pred_backward.copy(),
        "smooth_pred": train_pred_smooth.copy(),
        "blend_raw_pred": train_pred_blend_raw.copy(),
    }
    test_reference = {
        "initial_pred": test_pred_final.copy(),
        "raw_pred": test_pred_raw.copy(),
        "backward_pred": test_pred_backward.copy(),
        "smooth_pred": test_pred_smooth.copy(),
        "blend_raw_pred": test_pred_blend_raw.copy(),
    }

    iteration_rows: list[dict[str, object]] = [
        {
            "stage_order": 0,
            "stage_label": "stage0_08",
            "method": final_method,
            "train_raw_rmse": stage0_raw_rmse,
            "train_final_rmse": stage0_final_rmse,
            "train_raw_violations": stage0_raw_violations,
            "train_final_violations": stage0_final_violations,
            "test_raw_violations": stage0_test_raw_violations,
            "test_final_violations": stage0_test_final_violations,
        }
    ]
    current_train_iter = train_pred_final.copy()
    current_test_iter = test_pred_final.copy()
    best_stage_label = "stage0_08"
    best_stage_method = final_method
    best_train_pred = train_pred_final.copy()
    best_test_pred = test_pred_final.copy()
    best_train_rmse = stage0_final_rmse
    iterative_train_predictions: dict[str, np.ndarray] = {"stage0_08": train_pred_final.copy()}
    iterative_test_predictions: dict[str, np.ndarray] = {"stage0_08": test_pred_final.copy()}

    for iteration_no in range(1, ITERATIVE_REFINE_STEPS + 1):
        iter_result = run_iterative_refine_step(
            iteration_no=iteration_no,
            current_train_pred=current_train_iter,
            current_test_pred=current_test_iter,
            train_reference=train_reference,
            test_reference=test_reference,
        )
        stage_label = f"iter_{iteration_no}"
        iteration_rows.append(
            {
                "stage_order": iteration_no,
                "stage_label": stage_label,
                "method": str(iter_result["method"]),
                "train_raw_rmse": float(iter_result["train_raw_rmse"]),
                "train_final_rmse": float(iter_result["train_final_rmse"]),
                "train_raw_violations": int(iter_result["train_raw_violations"]),
                "train_final_violations": int(iter_result["train_final_violations"]),
                "test_raw_violations": int(iter_result["test_raw_violations"]),
                "test_final_violations": int(iter_result["test_final_violations"]),
                "feature_count": int(iter_result["feature_count"]),
                "update_std": float(iter_result["update_std"]),
            }
        )
        current_train_iter = np.asarray(iter_result["train_final"], dtype=float)
        current_test_iter = np.asarray(iter_result["test_final"], dtype=float)
        iterative_train_predictions[stage_label] = current_train_iter.copy()
        iterative_test_predictions[stage_label] = current_test_iter.copy()
        print(
            f"Iter {iteration_no}: method={iter_result['method']}, "
            f"train_final_rmse={float(iter_result['train_final_rmse']):.4f}"
        )
        if float(iter_result["train_final_rmse"]) < best_train_rmse:
            best_stage_label = stage_label
            best_stage_method = str(iter_result["method"])
            best_train_pred = current_train_iter.copy()
            best_test_pred = current_test_iter.copy()
            best_train_rmse = float(iter_result["train_final_rmse"])
    ...
    iteration_summary_df = pd.DataFrame(iteration_rows)
    selected_train_pred = best_train_pred.copy()
    selected_test_pred = best_test_pred.copy()
    ...
    print(
        f"Selected iterative stage={best_stage_label}, method={best_stage_method}, "
        f"train_rmse={selected_train_rmse:.4f}"
    )
    print(iteration_summary_df)
    ...
    print(
        f"Selected iterative violations train={selected_train_violations}, "
        f"test={selected_test_violations}"
    )

    submit_df = build_submit_df(test_ids, selected_test_pred)
    rmse_tag = int(round(selected_train_rmse * 10000))
    submit_path = os.path.join(SUBMIT_DIR, f"submit_csv_10_iterative_prediction_refine_{DATE}_{rmse_tag:04d}.csv")
    ...
    detail_df = pd.DataFrame(
        {
            ...
            "selected_stage": best_stage_label,
            "selected_method": best_stage_method,
            "smooth_lambda_choices": summarize_choice_counts(smooth_lambda_choices),
        }
    )
    for stage_label, stage_pred in iterative_test_predictions.items():
        detail_df[f"{stage_label}_prediction"] = stage_pred
    detail_path = os.path.join(SUBMIT_DIR, f"test_prediction_detail_10_iterative_prediction_refine_{DATE}.csv")
    detail_df.to_csv(detail_path, index=False, encoding="utf-8-sig")
    fold_summary_path = os.path.join(SUBMIT_DIR, f"fold_summary_10_iterative_prediction_refine_{DATE}.csv")
    fold_summary_df.to_csv(fold_summary_path, index=False, encoding="utf-8-sig")
    iteration_summary_path = os.path.join(SUBMIT_DIR, f"iteration_summary_10_iterative_prediction_refine_{DATE}.csv")
    iteration_summary_df.to_csv(iteration_summary_path, index=False, encoding="utf-8-sig")
    ...
    mlflow.log_params(
        {
            ...
            "smooth_lambda_choices": summarize_choice_counts(smooth_lambda_choices),
            "iterative_refine_steps": ITERATIVE_REFINE_STEPS,
            "iterative_update_rate": ITERATIVE_UPDATE_RATE,
            "selected_stage": best_stage_label,
            "selected_stage_method": best_stage_method,
        }
    )
    ...
    mlflow.log_metric("selected_train_rmse", selected_train_rmse)
    ...
    mlflow.log_metric("selected_train_violations", selected_train_violations)
    ...
    mlflow.log_metric("selected_test_violations", selected_test_violations)
    ...
    for _, row in iteration_summary_df.iterrows():
        mlflow.log_metric(f"{row['stage_label']}_train_final_rmse", float(row["train_final_rmse"]))
...
# %%
plot_raw_vs_final(train_meta_df, train_pred_raw, selected_train_pred, "Train", best_stage_label)
plot_raw_vs_final(test_meta_df, test_pred_raw, selected_test_pred, "Test", best_stage_label)
plot_prediction_trend_by_species(
    train_meta_df,
    selected_train_pred,
    f"学習データ: 樹種ごとの予測含水率の推移 ({best_stage_label})",
)
plot_prediction_trend_by_species(
    test_meta_df,
    selected_test_pred,
    f"テストデータ: 樹種ごとの予測含水率の推移 ({best_stage_label})",
)
plot_iteration_rmse(iteration_summary_df)
